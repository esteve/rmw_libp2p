...
pub struct Libp2pROS2Bridge {
    ...
    incoming_queue: Arc<deadqueue::unlimited::Queue<(Topic, Vec<u8>)>>,
}
...
impl Libp2pROS2Bridge {
    fn new() -> Self {
        ...
        let incoming_queue = Arc::new(deadqueue::unlimited::Queue::<(Topic, Vec<u8>)>::new());

        let incoming_queue_clone = Arc::clone(&incoming_queue);
